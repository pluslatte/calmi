...
async fn returns_create_activity_as_a_valid_create_object() {
    ...
    assert_eq!(json["actor"], author_id);
    ...
    assert_eq!(object["attributedTo"], author_id);
}
...
async fn multiple_notes_have_different_activities() {
    ...
    let response1 = server.get("/users/alice/notes/note1/activity").await;
    response1.assert_status_ok();
    let json1: Value = response1.json();

    let response2 = server.get("/users/alice/notes/note2/activity").await;
